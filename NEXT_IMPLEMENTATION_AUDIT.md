...
- **Offline sync — now wired for the technician's core write paths.** Device test results, deficiency create/edit, CAN/ULC-S536 checklist responses (`ChecklistCompletion.tsx`), and generic inspection-template responses (`TemplateFormRenderer.tsx`) all save through the same `useOfflineStorage.ts` (localStorage) queue/sync pattern, and SyncScreen pushes each to its matching tRPC mutation on reconnect. The one remaining gap: `FireAlarmInspection.tsx`/`SmokeAlarmInspection.tsx` use a separate IndexedDB store (`offlineStorage.ts`) for fire-alarm checklist items that is still not wired into SyncScreen (tracked in `OFFLINE_SYNC_AUDIT.md`). The service worker provides static-asset caching/installability, not data sync.
