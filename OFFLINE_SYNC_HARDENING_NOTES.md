...
### Smoke Alarm offline testing gap — CLOSED (v3)
Previously, `SmokeAlarmInspection.tsx` had no offline mechanism at all — `recordTest.mutate()` was called directly with no `isOnline` check, no queue, no fallback, so a connectivity drop mid-test silently errored. Now fixed with its own IndexedDB store (the data shape differs from fire alarm: per-alarm test result, keyed by the alarm device id, not per-checklist-item):

- `offlineStorage.ts` bumped to `DB_VERSION` 3, adding a `pendingSmokeAlarmTests` object store (indexed by `jobId`, `alarmId`, `synced`) alongside a `subscribePendingSmokeTests()` pub/sub. `savePendingSmokeTest` **upserts by `alarmId`** (deletes any still-unsynced result for the same alarm before adding) so re-recording an alarm can't stack duplicates — matching the server's update-by-id semantics. The v2→v3 upgrade is idempotent (`contains()`-guarded), so existing fire-alarm/photo queues survive.
- New `usePendingSmokeAlarmTests()` hook reads the live pending list reactively.
- `SmokeAlarmInspection.tsx`: `handleRecordTest` is now async with an offline branch (queue to IndexedDB + "saved offline" toast) and a server-failure fallback to the same queue. Offline-recorded results are **overlaid** on the server data via `getEffectiveResult()`, so the test badge appears and the pass/fail/no-access/N-A buttons hide immediately — even before sync. A reconnect `useEffect` flushes the queue via a callback-free `recordTest` mutation (so it doesn't fire a per-item success toast) and refetches once at the end. The header shows an offline indicator and an "N unsynced" count.
- `SyncScreen.tsx` shows a "Smoke Alarm Tests · Job #X" pending section, includes the count in the total/stat-card breakdown, syncs each via `trpc.smokeAlarm.recordTest` one-by-one (no bulk endpoint), and clears the store on "Clear All Offline Data".
- `OfflineBanner.tsx`'s pending badge/count now includes this queue.
- Both sync paths (SyncScreen and the per-page reconnect effect) are safe to coexist: `deleteSmokeTest` no-ops on an already-removed id, and `smokeAlarm.recordTest` is an idempotent update-by-id, so a double-sync at worst repeats one harmless upsert.
