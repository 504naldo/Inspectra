...
function InventoryImport() {
  ...
  const [previewData, setPreviewData] = useState<{ toCreate: number; toUpdate: number; skipped: number } | null>(null);
  const [importResult, setImportResult] = useState<ImportResult | null>(null);
  const [pendingRows, setPendingRows] = useState<InventoryRow[]>([]);
  const [showPreviewRows, setShowPreviewRows] = useState(false);

  const parseMutation   = trpc.importCenter.parseInventoryFile.useMutation();
  const previewMutation = trpc.inventory.importPreview.useMutation();
  const executeMutation = trpc.inventory.importExecute.useMutation();

  async function handleFile(file: File) {
    if (!file.name.match(/\.(xlsx|xls)$/i)) { toast.error("Please select an .xlsx or .xls file."); return; }
    setStep("parsing"); setParseResult(null); setPreviewData(null); setImportResult(null);
    try {
      const fileData = await toBase64(file);
      const result = await parseMutation.mutateAsync({ fileData, fileName: file.name });
      setParseResult(result as InventoryParseResult);
      const validRows = (result as InventoryParseResult).parsed.filter((r) => !r._dupWithin);
      setPendingRows(validRows);
      const prev = await previewMutation.mutateAsync({ rows: validRows, updateExisting });
      setPreviewData(prev);
      setStep("preview");
    } catch (err: any) {
      toast.error(err.message ?? "Failed to parse file."); setStep("error");
    }
  }
  async function handleExecute() {
    setStep("confirming");
    ...
    } catch (err: any) { toast.error(err.message ?? "Import failed."); setStep("error"); }
  }
  function handleReset() {
    setStep("idle"); setParseResult(null); setPreviewData(null); setImportResult(null);
    setPendingRows([]); setShowPreviewRows(false);
    if (fileRef.current) fileRef.current.value = "";
  }
  const isBusy = step === "parsing" || step === "confirming";
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {step === "idle" && (
          <>
            <p className="text-sm text-muted-foreground">
              Upload an Inventory workbook (.xlsx). Expected sheet: <span className="font-mono text-xs bg-muted px-1 rounded">Inventory</span>, data starts at row 5.
            </p>
            <div className="flex items-center gap-3">
              <label className="cursor-pointer">
          ...
          <div className="space-y-3">
            <div className="rounded border bg-muted/30 p-3 text-sm space-y-1">
              <div className="flex gap-4 flex-wrap">
                <span className="text-muted-foreground">Sheet: <span className="text-foreground font-mono text-xs">{parseResult.sheetUsed}</span></span>
                <span className="text-muted-foreground">Scanned: <span className="text-foreground font-semibold">{parseResult.totalScanned}</span> rows</span>
                <span className="text-muted-foreground">Valid items: <span className="text-foreground font-semibold">{parseResult.parsed.length}</span></span>
                {parseResult.dupWithinCount > 0 && (<span className="text-amber-600">Intra-file dupes skipped: {parseResult.dupWithinCount}</span>)}
              </div>
              <div className="flex gap-4 flex-wrap pt-1 border-t mt-1">
                <span className="text-green-700 font-medium">To create: {previewData.toCreate}</span>
                <span className="text-blue-700 font-medium">To update: {previewData.toUpdate}</span>
            ...
            {parseResult.parsed.length > 0 && (
              <button className="text-xs text-muted-foreground flex items-center gap-1 hover:text-foreground" onClick={() => setShowPreviewRows((v) => !v)}>
                {showPreviewRows ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
                {showPreviewRows ? "Hide" : "Show"} parsed rows ({parseResult.parsed.length})
              </button>
            )}
                  ...
                  <tbody>
                    {parseResult.parsed.map((r) => (
                      <tr key={r._rowIndex} className={`border-t ${r._dupWithin ? "opacity-40" : ""}`}>
                        ...
                        <td className="px-2 py-0.5 text-right">${r.unitCost.toFixed(2)}</td>
                        <td className="px-2 py-0.5 text-right">${r.unitPrice.toFixed(2)}</td>
                        <td className="px-2 py-0.5 text-right">{r.quantityOnHand}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {previewData.toCreate === 0 && previewData.toUpdate === 0 ? (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Info className="h-4 w-4" /> Nothing to import — all items already exist in inventory.
              </div>
            ) : (
              <div className="flex gap-2">
                <Button size="sm" onClick={handleExecute} disabled={step === "confirming"}>
                  {step === "confirming" ? (<><RefreshCw className="h-3.5 w-3.5 mr-1.5 animate-spin" /> Importing…</>) : (<>Confirm Import ({previewData.toCreate + previewData.toUpdate} items)</>)}
                </Button>
                <Button size="sm" variant="ghost" onClick={handleReset} disabled={step === "confirming"}>Cancel</Button>
              </div>
            )}
          </div>
        )}
