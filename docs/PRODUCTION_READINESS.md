...
| PR-11 | Offline/sync duplicate-protection + QA preflight hardening | P1 | Technician | in-progress | non-idempotent `deficiency.create`; QA submit could omit unsynced items | `fix: strengthen offline sync safeguards` + `feat: QA-submit preflight` | `offlineSyncIdempotency.test.ts` (5), `qaPreflight.test.ts` (4) | eng | Extend idempotency to smoke-alarm/photo sync; per-job (not global) pending scoping |
