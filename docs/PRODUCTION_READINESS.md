...
| PR-06 | Easy to omit company-ownership checks in routers | P1 | Security | fixed (core) | load-by-id + manual compare pattern | `security: add scoped tenant getters…` + 8× `security: adopt scoped tenant getters…` / `close two cross-tenant IDORs` | `authorization.test.ts` (12) | eng | Migrated the exact load-then-throw ownership checks to scoped getters across ~20 routers (job/site/device/customerOrg/invoice/quote/workOrder/partsCatalogItem/serviceAgreement/inventory). Guards in `tenantGuards.ts`: getJobForCompany, getInvoiceForCompany, getQuoteForCompany, assertSite/Device/CustomerOrg/WorkOrder/PartsCatalogItem/ServiceAgreement/InventoryItemCompany; local helpers (reportQa, inventory, knowledgeSystem) delegate to them. **Also closed 2 latent cross-tenant IDORs surfaced by the pass** (`sync.getJobDataForOffline`, `drive.saveReportToDrive`). Deliberately left: role-branched reads (entityRouters customer scoping), AI-context helpers that return `"(access denied)"` strings, `input.companyId` validations, and entities without a shared guard (partsRequest, purchaseOrder, vendor, knowledgeBase page/fact/model, serviceSchedule) |
