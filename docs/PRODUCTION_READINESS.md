...
| PR-12 | Customer report privacy not enforced by a central sanitizer/test | P1 | Privacy | fixed | PDF generators use typed projections (no raw rows) | `security: central customer-safe report sanitizer` | `customerSafeReport.test.ts` (5) | eng | Central allow-list `buildCustomerSafeReportData` (exclude-by-default) wired into inspection/deficiency report path + seeded-field regression test. Follow-up: extend the same serializer to compliance/invoice/quote PDFs |
