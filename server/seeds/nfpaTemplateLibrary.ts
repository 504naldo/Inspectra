/**
 * NFPA / ULC pre-built inspection-template library — source of truth.
 *
 * Extracted from scripts/seedNFPATemplates.ts (which now imports from here) so
 * the same definitions drive all three distribution paths:
 *   1. scripts/seedNFPATemplates.ts   — manual per-company CLI seeding
 *   2. server/seeds/provisionTemplates.ts — automatic install on company.create
 *   3. drizzle/migrations/0083_seed_nfpa_ulc_template_library.sql — back-fill of
 *      existing companies (generated by scripts/generatePrebuiltTemplateMigration.ts)
 *
 * NOTE: this library's fire-extinguisher entry ("NFPA 10 — Portable Fire
 * Extinguisher Annual Inspection") is EXCLUDED from paths 2 and 3 because the
 * library already ships the NFPA 10 template from seeds/nfpa10Extinguisher.ts
 * (deployed via migration 0082); shipping both would duplicate the form.
 *
 * `defaultAssignmentSystemType`/`defaultAssignmentJobType` are only honoured by
 * the manual CLI script. Automatic provisioning deliberately creates NO
 * assignments: the assignment matcher ignores systemType, so a systemType-only
 * assignment matches every job — auto-creating them would attach all ten
 * checklists to every job. Offices assign templates from the admin Templates UI.
 */
...
export const NFPA_TEMPLATE_LIBRARY: TemplateDef[] = [
